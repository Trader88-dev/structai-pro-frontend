...
export function PoutreContinueSchema({ travees, b, h, d }: PoutreContinueSchemaProps) {
  ...
  const maxM   = safeTravees.length > 0 ? Math.max(...safeTravees.flatMap(t => [Math.abs(t.M_trav), Math.abs(t.M_app_g), Math.abs(t.M_app_d)])) : 1
  const maxV   = safeTravees.length > 0 ? Math.max(...safeTravees.flatMap(t => [Math.abs(t.V_g), Math.abs(t.V_d)])) : 1

  useEffect(() => {
    if (!mountRef.current || !safeTravees.length) return
